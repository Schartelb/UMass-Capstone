...
const NavBar = ({ setLoginorSignup }) => {
    ...
    function loggedInNav() {
        return (
            <>
                <NavItem onClick={profileClick} className="nav">
                    <div>Profile</div>
                </NavItem >
                <NavItem className="nav" onClick={handleClick}><div>Logout</div></NavItem>
            </>
        )
    }
    return (
        <div>
            <Navbar expand="md">
                ...
                {userContext.currUser ? loggedInNav():loggedOutNav()}
                </Nav>
            </Navbar>
        </div>
    )
